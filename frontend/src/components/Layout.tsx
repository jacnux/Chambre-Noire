...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const isListPage = ['/dashboard', '/galleries', '/comments'].includes(location.pathname);
  ...
  return (
    <div className={`relative min-h-screen w-full ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      <div className="relative z-10 min-h-screen pb-20 flex flex-col">
        <div className={`sticky top-0 z-20 border-b p-3 sm:p-4 backdrop-blur-lg flex justify-between items-center gap-2 shadow-lg ${theme === 'dark' ? 'bg-gray-900/90 border-gray-700' : 'bg-white border-gray-200'}`}>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            {user ? (
              <>
                <div className={`flex gap-1 p-1 rounded-full border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200'}`}>
                  <Link to="/dashboard" className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition ${location.pathname === '/dashboard' ? 'bg-blue-600 text-white' : (theme === 'dark' ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-black')}`}>
                    📁 Albums
                  </Link>

                  <Link to="/galleries" className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition ${location.pathname === '/galleries' ? 'bg-purple-600 text-white' : (theme === 'dark' ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-black')}`}>
                    🖼️ Galeries
                  </Link>

                  <Link
                    to="/comments"
                    className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition flex items-center gap-1.5 ${isCommentsRoute ? 'bg-blue-600 text-white' : (theme === 'dark' ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-black')}`}
                  >
                    <span>💬</span>
                  ...
                  <Link
                    to="/manage-blog"
                    className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition flex items-center gap-1 ${location.pathname === '/manage-blog' ? 'bg-orange-600 text-white' : (theme === 'dark' ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-black')}`}
                  >
                    <span>📝</span> Mon Blog
                  </Link>
                  ...
                  <Link
                    to="/dashboard/pages"
                    className={`px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition flex items-center gap-1 ${location.pathname.startsWith('/dashboard/pages') ? 'bg-yellow-500 text-black' : (theme === 'dark' ? 'text-gray-400 hover:text-white' : 'text-gray-600 hover:text-black')}`}
                  >
                    <span>📄</span> Mes Pages
                  </Link>
                </div>
                ...
                <div className={`flex items-center gap-1 rounded-full p-1 border ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200'}`}>
                  {isAdmin && (
              ...
              <>
                <Link to="/login" className={`px-4 py-2 text-sm ${theme === 'dark' ? 'text-gray-300 hover:text-white' : 'text-gray-600 hover:text-black'}`}>
                  Connexion
                </Link>
