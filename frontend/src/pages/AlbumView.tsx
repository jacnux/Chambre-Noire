// =================================================
// 23 Mai 2026 - Version 8.8 + viewer allégé + retour portfolio via route state + fallback sous-domaine + barre sticky admin / barre fixe viewer
// AlbumView
// =================================================
...
const AlbumView = () => {
  ...
  const actionsBarWrapperClass = isViewer
    ? 'px-4 pt-3'
    : 'sticky top-[76px] sm:top-[88px] z-20 px-4 pt-4';
  ...
  const viewerBackLink = locationState?.portfolioPath || (subdomain ? '/' : null);
  ...
  return (
    <div className="relative min-h-screen w-full bg-gray-900">
      ...
      <div className="relative z-10 min-h-screen pb-20">
        {/* BARRE DE CONTEXTE — remplace l'ancien header local */}
        <div className="px-4 pt-4">
          <div className={`max-w-6xl mx-auto backdrop-blur border rounded-2xl shadow-sm ${
            isViewer
              ? 'bg-white/5 border-white/5 px-4 py-3'
              : 'bg-white/5 border-white/10 px-4 py-3'
          }`}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  {isViewer ? (
                    viewerBackLink ? (
                      <Link to={viewerBackLink} className="text-gray-400 hover:text-white transition">
                        {viewerBackLabel}
                      </Link>
                    ) : (
                    ...
                    <Link
                      to={album?.isVirtual ? '/galleries' : '/dashboard'}
                ...
                <div className="mt-1 flex flex-wrap items-center gap-2">
                  <h1 className={`font-bold text-white truncate ${
                    isViewer ? 'text-lg sm:text-xl' : 'text-xl sm:text-2xl'
                  }`}>
                    {album ? album.title : 'Chargement...'}
                  </h1>

                  {isViewer && (
                    <span className="text-[11px] px-2 py-1 rounded-full bg-white/8 text-gray-200 border border-white/10">
                      Public
                    </span>
                  )}

                  {album?.isVirtual && (
                    <span className="text-[11px] px-2 py-1 rounded-full bg-purple-500/15 text-purple-200 border border-purple-400/20">
                      ✦ Galerie virtuelle
                    </span>
                  )}
                </div>
                ...
                {album?.description && (
                  <p className={`text-gray-300 mt-2 line-clamp-2 ${
                    isViewer ? 'text-xs sm:text-sm' : 'text-sm'
                  }`}>
                    {album.description}
                  </p>
                )}
        ...
        {/* Zone Upload — uniquement pour albums non-virtuels */}
        {canAddPhotos && pendingFiles.length > 0 && (
          <div className="max-w-4xl mx-auto mt-6 bg-white/10 backdrop-blur-lg border border-white/20 p-6 rounded-2xl shadow-2xl">
            <h2 className="text-xl font-bold mb-4 text-white">
              Photos à envoyer ({pendingFiles.length})
            </h2>
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              {pendingFiles.map((pf, idx) => (
                <div
                  key={idx}
                  className="flex flex-col sm:flex-row gap-4 items-start border-b border-white/10 pb-4"
                >
                  <div className="w-full sm:w-auto flex justify-center relative">
                    <img
                      src={pf.previewUrl}
                      className="w-24 h-24 object-cover bg-black/30 rounded-lg"
                      alt="Aperçu"
                    />
                    <button
                      type="button"
                  ...
                  <div className="flex-1 w-full space-y-2">
                    <div className="flex gap-2">
                      <input
                        ...
                        value={pf.index}
                        onChange={e => {
                          const n = [...pendingFiles];
                          n[idx].index = parseInt(e.target.value);
                          setPendingFiles(n);
                        }}
                    ...
                    <div className="mt-2 bg-white/5 p-3 rounded-lg border border-white/10 space-y-2">
                      <label className="flex items-center gap-2 text-xs text-gray-300 cursor-pointer">
                        <input
                          ...
                          onChange={e => {
                            const n = [...pendingFiles];
                            n[idx].applyWatermark = e.target.checked;
                            setPendingFiles(n);
                          }}
                      ...
                      {pf.applyWatermark && (
                        <input
                          ...
                          onChange={e => {
                            const n = [...pendingFiles];
                            n[idx].watermarkText = e.target.value;
                            setPendingFiles(n);
                          }}
              ...
              <button
                type="submit"
                className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white py-3 rounded-full hover:opacity-90 font-bold shadow-lg"
              >
                Valider l'envoi
              </button>
            </form>
        ...
        {/* BARRE D'ACTIONS — sticky en admin, statique et allégée en viewer */}
        {hasPhotos && (
          <div className={actionsBarWrapperClass}>
            <div className={`max-w-6xl mx-auto backdrop-blur rounded-2xl flex justify-between items-center gap-3 ${
              isViewer
                ? 'bg-white/5 border border-white/5 px-3 py-2 shadow-sm'
                : 'bg-white/10 border border-white/20 px-3 py-3 shadow-lg flex-col sm:flex-row items-start sm:items-center'
            }`}>
              {!isViewer && (
              ...
              <div className={`flex flex-wrap items-center gap-2 ${isViewer ? 'w-full justify-between sm:justify-end' : 'w-full sm:w-auto'}`}>
                ...
                <div className={`rounded-full p-1 flex gap-1 ${
                  isViewer ? 'bg-white/5 border border-white/5' : 'bg-white/10 border border-white/10'
                }`}>
                  <button
                    onClick={() => setViewMode('grid')}
                    className={`transition ${isViewer ? 'px-3 py-1.5 text-xs' : 'p-2'} rounded-full ${
                      viewMode === 'grid'
                ...
                {!isViewer && (
                  <div className="bg-white/10 rounded-full p-1 flex gap-1 border border-white/10">
                    <button
                      ...
                      className={`p-2 rounded-full transition text-xs font-bold ${
                        sortMode === 'date_desc'
        ...
        {/* ÉTAT VIDE — bouton Ajouter simple, seulement si on peut uploader */}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-300 mt-1">
                    Ajoutez vos premières photos pour afficher la grille, la liste et les options de tri.
                  </p>
                </div>
        ...
        {/* ÉTAT VIDE — album virtuel sans résultats */}
        {album?.isVirtual && !hasPhotos && (
          <div className="px-4 py-10 text-center">
            <p className="text-purple-300 text-sm">
              ✦ Aucune photo ne correspond au filtre de cette galerie virtuelle.
            </p>
          </div>
        )}
          ...
          >
            {sortedPhotos.map((photo, idx) =>
              viewMode === 'grid' ? (
                <div
                  key={photo._id}
                  className="relative aspect-square bg-black/20 rounded-xl overflow-hidden group cursor-pointer"
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-100 transition duration-300 flex flex-col justify-end p-3 pointer-events-none">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {Array.isArray(photo.tags) &&
                        photo.tags.slice(0, 3).map((tag: string, tIdx: number) => (
                          <span
                            key={tIdx}
                            className="bg-white/20 backdrop-blur-sm text-white text-[10px] px-1.5 py-0.5 rounded"
                          >
                            #{tag}
                          </span>
                        ))}
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 z-20">
                    <button
                      ...
                      className="w-8 h-8 bg-blue-600/80 backdrop-blur rounded-full shadow-lg text-white hover:bg-blue-600 flex items-center justify-center text-sm"
                      title="Commenter"
                    >
                      💬
                    </button>
                ...
                <div
                  key={photo._id}
                  className="bg-white/5 backdrop-blur border border-white/10 rounded-xl p-3 flex items-center gap-4 hover:bg-white/10 transition group"
                >
                  <div
                    className="w-16 h-16 flex-shrink-0 rounded-lg overflow-hidden bg-black/20 cursor-pointer"
                    onClick={() => setLightboxIndex(idx)}
                  >
                    <img
                      src={`/uploads/${photo.filename}`}
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {Array.isArray(photo.tags) &&
                        photo.tags.slice(0, 3).map((tag: string, tIdx: number) => (
                          <span key={tIdx} className="text-purple-300 text-[10px]">
                            #{tag}
                          </span>
                        ))}
                    </div>
